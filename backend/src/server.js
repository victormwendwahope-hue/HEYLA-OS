...
app.post('/api/payroll/export-payslips', requireAuth, async (req, res) => {
  try {
    ...
    const fields = [
      ...
      { key: 'overtime', header: 'Overtime (OT1)' },
      { key: 'overtime2', header: 'Public Holiday (OT2)' },
...
app.post('/api/payroll/generate-payslip', requireAuth, async (req, res) => {
  try {
    ...
    const payslip = {
      ...
      overtime: Math.round((record.overtime || 0) * (record.hourlyRate || 0) * 1.5),
      overtime2: Math.round((record.overtime2 || 0) * (record.hourlyRate || 0) * 2.0),
      grossPay: gross,
      paye,
      nssf,
      nhif,
