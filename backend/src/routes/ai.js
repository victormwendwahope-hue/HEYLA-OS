...
const heylaKeywords = Object.keys(heylaResponses);

function getHeylaResponse(input) {
  const lower = input.toLowerCase();
  for (const key of heylaKeywords) {
    if (lower.includes(key)) return heylaResponses[key];
  }
...
router.post('/chat', async (req, res) => {
  try {
    ...
    const response = getHeylaResponse(message.trim());
