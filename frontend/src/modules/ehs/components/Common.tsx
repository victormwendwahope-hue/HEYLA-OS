...
import { RiskBand, IncidentSeverity } from '@/modules/ehs/types';
import { riskMeta, severityVariant, initialsOf, avatarHue } from '@/modules/ehs/utils/format';
...
export function SeverityBadge({ severity }: { severity: IncidentSeverity }) {
  return <Badge text={severity} variant={severityVariant(severity)} />;
}
...
export function Money({ amount, currency = 'KEN' }: { amount: number; currency?: string }) {
  const formatted = new Intl.NumberFormat('en', { style: 'currency', currency, maximumFractionDigits: 0 }).format(amount);
